...
export async function GET(request) {
  ...
  if (!recipientEmail) return Response.json({ error: 'DAILY_REPORT_EMAIL nao configurado' }, { status: 500 });
...
REGRAS OBRIGATORIAS:
- Responda DIRETO sem avisar que vai pesquisar
- NUNCA use simbolos Markdown: sem **, sem ##, sem --, sem *
- Use LETRAS MAIUSCULAS para titulos
...
3) MELHORES OPORTUNIDADES - top 3 ativos para comprar agora com tickers especificos e motivo
4) O QUE EVITAR - ativos ou setores a evitar esta semana e por que
...
<tr><td style="background:linear-gradient(135deg,#1E3A8A,#2563EB);border-radius:16px 16px 0 0;padding:32px 36px;">
  ...
  <div style="color:rgba(255,255,255,.75);font-size:13px;margin-top:6px;">Relatorio Automatico Diario - ${today}</div>
</td></tr>
<tr><td style="background:white;padding:24px 36px 0;">
  <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:14px 18px;">
    ...
    <div style="font-size:11px;color:#3B82F6;margin-top:2px;">Relatorio gerado automaticamente com dados atuais</div>
  </div>
</td></tr>
...
<tr><td style="background:#FFFBEB;border-top:1px solid #FDE68A;padding:14px 36px;">
  <p style="font-size:12px;color:#92400E;">Aviso: Analise por IA com fins informativos. Nao constitui recomendacao formal de investimento.</p>
</td></tr>
<tr><td style="background:#1E293B;border-radius:0 0 16px 16px;padding:20px 36px;text-align:center;">
  <div style="color:#94A3B8;font-size:11px;">InvestIA Dashboard - Envio automatico - ${today} - Powered by Claude Sonnet</div>
</td></tr>
